...
IF EXISTS (SELECT * 
		   FROM SYSOBJECTS 
		   WHERE ID = OBJECT_ID('dbo.CUP_SPP_SaldosMenoresSL') AND 
				 TYPE = 'P') BEGIN
				 
	 DROP PROCEDURE dbo.CUP_SPP_SaldosMenoresSL
...
/*
 ...
 EXEC CUP_SPP_SaldosMenoresSL 
          ...
          @ToleranciaKGS = 1, --  Regresa solo los saldos menores bajo la tolerancia especificad.
          @EnSilencio  = 0
*/

CREATE PROCEDURE CUP_SPP_SaldosMenoresSL
  @Usuario CHAR(10),
  @Empresa CHAR(5) = 'CML',
  @Sucursal INT = NULL,
...
AS BEGIN TRY
  ...
  SELECT
    ...
    sl.Propiedades,
    sl.Existencia,
    ExistenciaReal = ROUND(ISNULL(sl.Existencia,0),4,1),
    ExistenciaRemanente = ISNULL(sl.Existencia,0) -  ROUND(ISNULL(sl.Existencia,0),4,1)
  FROM
    SerieLote sl
  JOIN art a  ON sl.Articulo = a.Articulo
  WHERE
    a.UnidadTraspaso = 'KGS'
  ...
  AND ISNULL(sl.Existencia,0) < ISNULL(@ToleranciaKGS,0)
  AND a.Categoria IN('ALUMINIO','ACERO INOX')
  AND a.Grupo IN('Lamina','Placa','Placa Alum','')
  --AND a.Articulo = 'A0180480NAT3003H14-R'
  --AND sl.SubCuenta = 'L13112M1'
